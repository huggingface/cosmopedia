...
json_regex = "\{\"\s{0,16}bisac_category\":\s{0,16}\"<BISAC_REGEX>\",\s{0,16}\"text_type\":\s{0,16}\"(?:book|web)\"\s{0,16}\}"
...
* "bisac_category": "ARCHITECTURE / Historic Preservation / Restoration Techniques"
* "bisac_category": "COMPUTERS / Data Science / General"
* "bisac_category": "DESIGN / Graphic Arts / Illustration"
* "bisac_category": "TECHNOLOGY & ENGINEERING / Electronics / Circuits / Integrated"
* "bisac_category": "NON-CLASSIFIABLE"
...
You are only allowed to reply with a valid json object in the following format:
```
...
def main(args):
    ...
    def classify_text(sample):
        ...
        label = json.loads(response.generated_text)
    ...
    dataset.push_to_hub("HuggingFaceFW/FW-2023-50-bisac-classified", private=True)
